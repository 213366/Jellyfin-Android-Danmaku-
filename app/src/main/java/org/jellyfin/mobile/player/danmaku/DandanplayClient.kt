 ...
 *
 * 默认经由公共 CORS 代理访问（代理负责附加 dandanplay 的 AppId/签名），
 * 也可通过自定义 API 地址指向自建代理或 misaka_danmu_server。
 */
class DandanplayClient(okHttpClient: OkHttpClient) {
    ...
    /**
     * 按番剧名称搜索剧集，对应 `/api/v2/search/episodes`
     */
    ...
    /**
     * 获取指定剧集的弹幕（含第三方关联源），对应 `/api/v2/comment/{episodeId}?withRelated=true`
     */
     ...
     * 对应 `{server}/api/danmu/{itemId}/raw`，失败返回 null。
     *
     * XML 格式：`<d p="stime,type,fontSize,color,date,pool,sender,dbid">内容</d>`
     */
        ...
        while (event != XmlPullParser.END_DOCUMENT) {
            when (event) {
                ...
                XmlPullParser.END_TAG -> if (parser.name == "d") {
                    val p = currentP
                    if (p != null) {
                        // 转换为 dandanplay 的 "time,mode,color,user" 格式
                        val parts = p.split(',')
                        if (parts.size >= 4) {
